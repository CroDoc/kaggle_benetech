...
def parse_opt():
    ...
    parser.add_argument(
        '--extracted', default=3, action='store', type=int, required=False
    )
...
# add extracted to train
dfs = [train_df]
for _ in range(opt.extracted):
    dfs.append(valid_df)

train_df = pd.concat(dfs).reset_index(drop=True)

# set skip validation to large number
cfg['skip_validation'] = 10000000
